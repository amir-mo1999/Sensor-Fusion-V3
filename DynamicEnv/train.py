...
if __name__=='__main__':
    ...
    eval_callback = EvalCallback(eval_env, best_model_save_path='./models/best_reach_ppo/',
                       log_path='./models/best_reach_ppo/', eval_freq=5120,
                       deterministic=True, render=False)
    
    # Save a checkpoint every ? steps
    checkpoint_callback = CheckpointCallback(save_freq=5120, save_path='./models/reach_ppo_ckp_logs/',
                                        name_prefix='reach')
    ...
    # model = PPO.load('./models/reach_ppo_ckp_logs/reach_5079040_steps', env=env) # load pretrained model
    model.learn(
        total_timesteps=1e10,
        n_eval_episodes=64,
        callback=callback,
        reset_num_timesteps=False)  # False when you do not want to reset timesteps to 0
